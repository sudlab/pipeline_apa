...
"""
from ruffus import transform, merge, collate, suffix, add_inputs, split, follows, mkdir, active_if, formatter, subdivide, regex, inputs
...
@transform("*.bam", formatter(),
           add_inputs(PARAMS["annotations_geneset_gtf"]),
...
@merge([assembleWithStringTie,
        PARAMS["annotations_geneset_gtf"]],
...
def generateDaParsTranscriptsToGenes(infile, outfile):
    ...
    IOTools.write_lines(outfile, outlines,
...
def bam_to_bedGraph(infile, outfile):
    ...
    genome_file = PARAMS["annotations_contigs_tsv"]
...
def generate_config(infiles, utrs, outfile):
    ...
    config_template = IOTools.open_file(template_file).read()
...
if not os.path.exists("design.tsv"):
    ...
    @subdivide("design.tsv",
               ...
               ["dapars_out.dir/%s.dapars_config.txt" % line.split()[0]
                for line in IOTools.open_file("design.tsv")
    ...
    def generate_dapars_config(infiles, outfile):
        ...
        comparisons = [line.split() for line in IOTools.open_file("design.tsv")
                       if not line.startswith("#")]

        for name, pat1, pat2 in comparisons:
            condition1_files = [f for f in bedgraphs if re.match(pat1, f)]
            condition2_files = [f for f in bedgraphs if re.match(pat2, f)]
            generate_config([condition1_files, condition2_files], utrs, 
                            "dapars_out.dir/%s.dapars_config.txt" % name)
...
@transform(bam_to_bedGraph,
           formatter(),
           add_inputs(PARAMS["annotations_contigs_tsv"]),
...
@transform(PARAMS["annotations_geneset_gtf"],
           formatter(),
...
def filter_overlapping_genes(infile, outfile):
    ...
    tmp2=P.get_temp_filename(shared=True, suffix=".gtf.gz")
    ...
    with IOTools.open_file(outfile, "w") as outf:
        for transcript in GTF.transcript_iterator(
                GTF.iterator(IOTools.open_file(tmp2))):
            nexon = 0
...
if os.path.exists("design.tsv"):
    @follows(mkdir("alt_utr_analysis.dir"))
    @split("design.tsv", "alt_utr_analysis.dir/*.design.txt")
    def generate_dexseq_design_files(infile, outfiles):
        ...
        for name, pat1, pat2 in comparisons:
            ...
            condition2_files = [(f, "Control") for f in bamfiles
                                if re.match(pat2, f)]
            IOTools.write_lines("alt_utr_analysis.dir/%s.design.txt" % name,
    ...
    def generate_dexseq_design_files(infiles, outfile):
        ...
        IOTools.write_lines(outfile, files, header=["track","condition"])
...
def count_chunks(infiles, outfile):
    ...
    statement = '''featureCounts -f -O -T 2 --primary -p -B -C 
                                 -a %(gtffile)s 
                                -o %(outfile)s %(bamfile)s'''
    P.run(statement, job_thrads=2, job_memory="6G")
...
def merge_chunk_counts(infiles, outfile):
    ...
    key_columns = "<(grep -v '^#' %s | cut -f1-6 ) " % infiles[0]

    infiles = key_columns + " ".join("<(grep -v '^#' %s | cut -f7 )" % f for f in infiles)

    job_memory = "10G"
    
    statement=''' paste %(infiles)s
               | sed '0,/\\.bam/{s/\\.bam//g}'
               | gzip > %(outfile)s '''
...
def get_last_exons(infile, outfile):
    ...
    gtfs = GTF.iterator(IOTools.open_file(infile))
    with IOTools.open_file(outfile, "w") as outf:
...
def load_last_exon_chunks(infile, outfile):
    ...
    with P.get_temp_file(shared=True) as tmpfile:
        tmpfile.write("gene_id\tchunk_id\n")
        for exon in GTF.iterator(IOTools.open_file(infile)):
            tmpfile.write("\t".join([exon.gene_id, re.sub(";","",str(exon["exon_id"]))])+"\n")
        tmpfn = tmpfile.name
